...
이 문서는 웹 요청 보안의 구현 계약(API·적재·계산 정책과 테스트 범위)의 단일 원본이다. 이 도메인을 바꾸는 PR은 코드·테스트와 함께 이 파일을 고친다. 제품 전체의 불변 계약은 [PROJECT_CONTEXT.md §6](../PROJECT_CONTEXT.md#6-변경하면-안-되는-계약), 코드 지도는 [ARCHITECTURE.md](../ARCHITECTURE.md)를 따른다.
...
- 계획 생성·수정·삭제, 계산, 후보 조회, 상품 조건 적용은 사용자별 프로세스 내 요청 빈도 제한을 둔다. 이 제한은 단일 인스턴스 방어선이며, 다중 인스턴스 운영에서는 배포 플랫폼의 공유·엣지 rate limit을 함께 설정해야 한다.
